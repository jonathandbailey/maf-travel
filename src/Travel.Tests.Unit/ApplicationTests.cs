...
public class ApplicationTests
{
    ...
    public async Task TravelPlanAgent_WhenProvidedWithCompletePlan_ShouldCompleteTheWorkflow(PlanningWorkflowScenario scenario)
    {
        ...
        foreach (var message in scenario.Runs)
        {
            using var runActivity = TestActivitySource.StartActivity($"Run {++runIndex}");

            var responses = await conversationAgent.RunStreamingAsync(message.Message, agentSession, options: agentRunOptions,
